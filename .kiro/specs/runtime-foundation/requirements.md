...
| 後続の実行機能 | 引数・localsを用いる実行、複数戻り値、数値演算、完全な制御構文、分岐、call、unreachable、globalsは`numeric-control`で扱う。メモリ・テーブル・参照命令・ホスト連携・start・SIMD命令はそれぞれの後続仕様で扱う。 |
| 公式適合検証 | 公式テスト素材の固定・変換からJSONによる実行・集計までは`conformance-runner`で扱い、spectestなどの対応は後続機能で同じツールへ追加する。本仕様の完了にこれらの完成は要求しない。 |
...
2. When 利用者が数値のvalueを明示的に構築して同じ型で取得した場合, the WasmSharp2 shall 元のビット列を保持し、浮動小数点数では符号付き0、無限大、NaNの符号とpayloadを区別できるようにする。
...
10. When 定義関数への入口またはstart実行がWasm実行コンテキストを開いた場合, the WasmSharp2 shall 定義関数の所属instanceまたはstartを持つinstanceの上限をそのコンテキストの終了まで固定し、内側で呼ばれたinstanceの上限は適用しない。
11. When 進行中のWasm実行から同じスレッド上で同期的にネストした関数呼び出しを行う場合, the WasmSharp2 shall ホストコールバックからの再呼び出しも含めて現在のWasm実行コンテキストを共有し、そのコンテキストを開いた実行が終了したときに後の独立した実行へ残さない。
...
host-linkingでは、既存コンテキストがない状態の単独ホスト呼び出しは、instanceの指定・省略やリソース操作だけではコンテキストを開始せず、そこからWasm定義関数またはstartへ入る時点で開始する。開始したWasm実行が終了してホストへ戻った後に別のWasm実行へ入る場合は、新しい入口のinstanceの上限を使う。callbackのInstance引数と深さ管理を分離し、Instance引数をWasmの関数型・値引数や実行ポリシーの選択に使わない。ホスト関数へのinstance明示と両callback形式の公開操作・受入条件は[host-linkingの要件](../host-linking/requirements.md)が所有する。これは基盤の定義関数の呼び出し契約や完了済みの受入範囲を変更しない。
